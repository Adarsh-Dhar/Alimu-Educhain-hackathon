...
const CourseCard: React.FC<Course & { onDelete?: (id: string) => void }> = ({ 
  id, 
  title, 
  description, 
  startTime, 
    ...
    <Card className="max-w-sm border shadow-md hover:shadow-lg transition-shadow duration-300">
      ...
      <CardFooter className="flex justify-between items-center">
        ...
        <Button 
          variant="destructive" 
          size="sm" 
          onClick={() => onDelete && onDelete(id.toString())}
        >
          Delete
        </Button>
      </CardFooter>
...
export const GetMyCoursesLearner: React.FC = () => {
  ...
  const { getMyCoursesLearner, removeCourse } = interaction();

  const renderCourses = async () => {
    try {
      const tx = await getMyCoursesLearner();
      if (!tx) {
        return null;
      }
